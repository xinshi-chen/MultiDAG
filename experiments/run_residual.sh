...
for i in ${!K_vals[*]}; do
  ...
  for baseline_ld in "${baseline_ld_vals[@]}"; do
    for threshold in "${threshold_vals[@]}"; do
      echo "######################################################################"
      echo "Recovering SERGIO for K-${K} n-${n} baseline_ld-${baseline_ld} thresh-${threshold}"
      echo "######################################################################"
      python residual_linear.py --real_dir ${real_dir} --group_size ${K} \
      --group_start 0 -K ${K} -p ${p} -n_sample ${n} -ld ${baseline_ld} -gpu 0 \

      python sergio_comparison.py --real_dir ${real_dir} -K ${K}\
      --threshold ${threshold} -ld ${baseline_ld} --baseline
    done
  done
done
